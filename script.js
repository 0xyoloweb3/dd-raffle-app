...
const modeTabsOpacityControls = document.getElementById('mode-tabs-opacity-controls');
const modeTabsOpacityDecreaseBtn = document.getElementById('mode-tabs-opacity-decrease');
const modeTabsOpacityIncreaseBtn = document.getElementById('mode-tabs-opacity-increase');
const modeTabsOpacityDrag = document.getElementById('mode-tabs-opacity-drag');
...
const headerMetaItems = Array.from(document.querySelectorAll('[data-header-meta-item]'));
...
const poolLabelDecreaseBtn = document.getElementById('pool-label-decrease');
const poolLabelIncreaseBtn = document.getElementById('pool-label-increase');
const historyLabelDecreaseBtn = document.getElementById('history-label-decrease');
const historyLabelIncreaseBtn = document.getElementById('history-label-increase');
const modeLabelDecreaseBtn = document.getElementById('mode-label-decrease');
const modeLabelIncreaseBtn = document.getElementById('mode-label-increase');
...
const HEADER_META_ITEMS_LAYOUT_KEY = 'rollbria-header-meta-items-layout';
...
const MODE_TABS_FILL_OPACITY_KEY = 'rollbria-mode-tabs-fill-opacity';
...
let headerMetaItemsState = {};
let headerMetaTextScale = 1;
let activeHeaderMetaDrag = null;
let activeHeaderMetaItemDrag = null;
...
let modeTabsScale = 1;
let modeTabsFillOpacity = 0.22;
let modeTabsOutlineSize = 1;
let activeModeTabsOpacityDrag = null;
...
function applyModeTabsLayout() {
  ...
  document.documentElement.style.setProperty('--mode-tab-hover-fill-opacity', `${modeTabsFillOpacity || 0.22}`);
...
function persistModeTabsFillOpacity() {
  localStorage.setItem(MODE_TABS_FILL_OPACITY_KEY, String(modeTabsFillOpacity));
}
...
function setModeTabsFillOpacity(nextOpacity) {
  modeTabsFillOpacity = Number(clamp(nextOpacity, 0.02, 0.95).toFixed(3));
  applyModeTabsLayout();
  persistModeTabsFillOpacity();
}

function updateModeTabsFillOpacity(delta) {
  setModeTabsFillOpacity((Number.isFinite(modeTabsFillOpacity) ? modeTabsFillOpacity : 0.22) + delta);
}
...
function applyHeaderMetaLayout() {
  ...
  headerMetaItems.forEach((item) => {
    const key = item.dataset.headerMetaItem;
    const state = headerMetaItemsState[key] || { x: 0, y: 0, scale: 1 };
    item.style.setProperty('--header-meta-item-x', `${state.x}px`);
    item.style.setProperty('--header-meta-item-y', `${state.y}px`);
    item.style.setProperty('--header-meta-item-scale', `${state.scale ?? 1}`);
  });
}
...
function saveHeaderMetaItemsLayout() {
  try {
    localStorage.setItem(HEADER_META_ITEMS_LAYOUT_KEY, JSON.stringify(headerMetaItemsState));
  } catch (_) {}
}
...
function updateHeaderMetaItemScale(key, delta) {
  const current = headerMetaItemsState[key] || { x: 0, y: 0, scale: 1 };
  headerMetaItemsState[key] = {
    ...current,
    scale: Number(clamp((Number.isFinite(current.scale) ? current.scale : 1) + delta, 0.5, 3).toFixed(3)),
  };
  applyHeaderMetaLayout();
  saveHeaderMetaItemsLayout();
}
...
function setupHeaderMetaDrag() {
  ...
  headerMetaItems.forEach((item) => {
    const key = item.dataset.headerMetaItem;
    headerMetaItemsState[key] = headerMetaItemsState[key] || { x: 0, y: 0, scale: 1 };

    item.addEventListener('pointerdown', (event) => {
      if (event.button !== 0) return;
      event.stopPropagation();
      activeHeaderMetaItemDrag = {
        pointerId: event.pointerId,
        key,
        startX: event.clientX,
        startY: event.clientY,
        originX: headerMetaItemsState[key].x || 0,
        originY: headerMetaItemsState[key].y || 0,
      };
    ...
    item.addEventListener('pointermove', (event) => {
      if (!activeHeaderMetaItemDrag || activeHeaderMetaItemDrag.pointerId !== event.pointerId || activeHeaderMetaItemDrag.key !== key) return;
      headerMetaItemsState[key] = {
        ...headerMetaItemsState[key],
        x: activeHeaderMetaItemDrag.originX + (event.clientX - activeHeaderMetaItemDrag.startX),
        y: activeHeaderMetaItemDrag.originY + (event.clientY - activeHeaderMetaItemDrag.startY),
      };
      applyHeaderMetaLayout();
      event.stopPropagation();
    });

    const finishHeaderMetaItemDrag = (event) => {
      if (!activeHeaderMetaItemDrag || activeHeaderMetaItemDrag.pointerId !== event.pointerId || activeHeaderMetaItemDrag.key !== key) return;
      item.classList.remove('is-dragging');
      saveHeaderMetaItemsLayout();
      activeHeaderMetaItemDrag = null;
      event.stopPropagation();
    };

    item.addEventListener('pointerup', finishHeaderMetaItemDrag);
    item.addEventListener('pointercancel', finishHeaderMetaItemDrag);
    item.addEventListener('dblclick', (event) => {
      event.preventDefault();
      event.stopPropagation();
      headerMetaItemsState[key] = { x: 0, y: 0, scale: 1 };
      applyHeaderMetaLayout();
      saveHeaderMetaItemsLayout();
    });
  });
...
function load() {
  try {
    ...
    const storedHeaderMetaItemsLayout = localStorage.getItem(HEADER_META_ITEMS_LAYOUT_KEY);
    ...
    if (storedHeaderMetaItemsLayout) {
      const parsedHeaderMetaItemsLayout = JSON.parse(storedHeaderMetaItemsLayout);
      if (parsedHeaderMetaItemsLayout && typeof parsedHeaderMetaItemsLayout === 'object') {
        headerMetaItemsState = parsedHeaderMetaItemsLayout;
      }
    }
    ...
    if (storedModeTabsFillOpacity) {
      const parsedModeTabsFillOpacity = Number(storedModeTabsFillOpacity);
      if (Number.isFinite(parsedModeTabsFillOpacity)) {
        modeTabsFillOpacity = parsedModeTabsFillOpacity;
      }
    }
...
if (poolLabelDecreaseBtn) {
  poolLabelDecreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateHeaderMetaItemScale('pool-label', -0.08);
  });
}
if (poolLabelIncreaseBtn) {
  poolLabelIncreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateHeaderMetaItemScale('pool-label', 0.08);
  });
}
if (historyLabelDecreaseBtn) {
  historyLabelDecreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateHeaderMetaItemScale('history-label', -0.08);
  });
}
if (historyLabelIncreaseBtn) {
  historyLabelIncreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateHeaderMetaItemScale('history-label', 0.08);
  });
}
if (modeLabelDecreaseBtn) {
  modeLabelDecreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateHeaderMetaItemScale('mode-label', -0.08);
  });
}
if (modeLabelIncreaseBtn) {
  modeLabelIncreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateHeaderMetaItemScale('mode-label', 0.08);
  });
}
if (modeTabsOpacityDecreaseBtn) {
  modeTabsOpacityDecreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateModeTabsFillOpacity(-0.03);
  });
}
if (modeTabsOpacityIncreaseBtn) {
  modeTabsOpacityIncreaseBtn.addEventListener('click', (event) => {
    event.preventDefault();
    updateModeTabsFillOpacity(0.03);
  });
}
if (modeTabsOpacityControls) {
  const beginModeTabsOpacityDrag = (event) => {
    if (event.button !== 0) return;
    if (event.target instanceof Element && event.target.closest('.mode-tabs-control-btn')) return;
    event.preventDefault();
    activeModeTabsOpacityDrag = {
      pointerId: event.pointerId,
      startX: event.clientX,
      startOpacity: Number.isFinite(modeTabsFillOpacity) ? modeTabsFillOpacity : 0.22,
    };
    modeTabsOpacityControls.classList.add('is-dragging');
    if (modeTabsOpacityDrag) {
      modeTabsOpacityDrag.classList.add('is-dragging');
    }
    modeTabsOpacityControls.setPointerCapture(event.pointerId);
  };

  const moveModeTabsOpacityDrag = (event) => {
    if (!activeModeTabsOpacityDrag || event.pointerId !== activeModeTabsOpacityDrag.pointerId) return;
    event.preventDefault();
    const deltaX = event.clientX - activeModeTabsOpacityDrag.startX;
    setModeTabsFillOpacity(activeModeTabsOpacityDrag.startOpacity + deltaX * 0.0025);
  };

  const stopModeTabsOpacityDrag = (event) => {
    if (!activeModeTabsOpacityDrag || event.pointerId !== activeModeTabsOpacityDrag.pointerId) return;
    modeTabsOpacityControls.classList.remove('is-dragging');
    if (modeTabsOpacityDrag) {
      modeTabsOpacityDrag.classList.remove('is-dragging');
    }
    if (modeTabsOpacityControls.hasPointerCapture(event.pointerId)) {
      modeTabsOpacityControls.releasePointerCapture(event.pointerId);
    }
    activeModeTabsOpacityDrag = null;
  };

  modeTabsOpacityControls.addEventListener('pointerdown', beginModeTabsOpacityDrag);
  modeTabsOpacityControls.addEventListener('pointermove', moveModeTabsOpacityDrag);
  modeTabsOpacityControls.addEventListener('pointerup', stopModeTabsOpacityDrag);
  modeTabsOpacityControls.addEventListener('pointercancel', stopModeTabsOpacityDrag);
  modeTabsOpacityControls.addEventListener('wheel', (event) => {
    if (event.target instanceof Element && event.target.closest('.mode-tabs-control-btn')) return;
    event.preventDefault();
    updateModeTabsFillOpacity(event.deltaY < 0 ? 0.03 : -0.03);
  }, { passive: false });
}
